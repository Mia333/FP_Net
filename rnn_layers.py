...
def rnn_step_forward(x, prev_h, Wx, Wh, b):
    ...
    ##############################################################################
    h_raw = prev_h.dot(Wh) + x.dot(Wx) + b
    next_h = np.tanh(h_raw)
    print(np.dot(x, Wx).shape)
    cache = (x, prev_h, h_raw, Wx,Wh)
    ##############################################################################
    #                               END OF YOUR CODE                             #
    ##############################################################################
    return next_h, cache
...
def rnn_forward(x, h0, Wx, Wh, b):
    ...
    for i in range(T):
        x_i = x[:,i,:]
        next_h, cache_i = rnn_step_forward(x_i, prev_h, Wx, Wh, b)
        prev_h = next_h.copy()
        h[:,i,:] = next_h.copy()
...
#   dnext_h = dh[:,T-1,:]
    ...
    for i in range(T):
        cache_i=cache_cp.pop()
        dnext_h = dh[:,T-1-i,:] + dprev_h
        dx[:,T-1-i,:], dprev_h, dWx_step, dWh_step, db_step = rnn_step_backward(dnext_h, cache_i) # dx[:,T-1-i]
...
def lstm_step_forward(x, prev_h, prev_c, Wx, Wh, b):
    ...
    cache = (x,H, prev_h, prev_c, Wx, Wh, b, a, i, f, o, g, next_c, next_h)
...
def lstm_step_backward(dnext_h, dnext_c, cache):
    ...
    #############################################################################
    x, H, prev_h, prev_c, Wx, Wh, b, a, i, f, o, g, next_c, next_h = cache
