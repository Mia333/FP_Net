...
class Model():
  def __init__(self, args, sampling=False, bias=0.):
    ...
    def loss_mse(evidence, predictions, weights):
      ...
      convergence_loss = tf.reduce_mean(convergence_loss_3d(predictions))
    ...
    def coeff_map(cell_outputs, num_mixture=20, bias=0.):
      ...
      pi, m1, m2, m3, s1, s2, s3, c12, c13, c23 = tf.split(
        axis=1, num_or_size_splits=10, value=outputs) 
    ...
    def loss_gmm(evidence, pi, mu, sigma, corr, weights):
      ...
      def full_loss(x1, x2, x3, pi, m1, m2, m3, s1, s2, s3, c12, c13, c23):
        ...
        gmm_loss = tf.maximum(gmm_loss, 1e-4) # avoid nan
      ...
      loss_final = loss_per_step + args.constraint_factor * convergence_loss
  ...
  def sample(self, sess, obstacle_traj, conditions, gmm=False, noise_level=0.001):
    ...
    # sample cluster in range(len(pi))
    ...
    # by default, the starting accelerations are all 0, [1,1,6]
    initial_inputs = np.zeros((1, 1, 6), dtype=np.float32)
    initial_inputs[0, 0, :3] = target - x_start # + step_noise()
    initial_inputs[0, 0, 3:] = repulsive_force(obstacle_traj[0, 0, :] - x_start)#+ step_noise() # obstacle_traj: [1, subseq_length, 3]
    ...
    prev_inp[0, 0, 3:] = repulsive_force(obstacle_traj[0, 1, :] - x_new) #+ step_noise() ## calculate the distance
                                                                         # between human and human(obstacle) 
                                                                         # obstacle_traj: (1 , , )
    v_last = v_new
    x_last = x_new
    ...
    for i in range(obstacle_traj.shape[1] - 1):
      feed = {tensor: s 
              for tensor, s in zip(nest.flatten(self.state_in), prev_state)} # self.state_in = cell.zero_state(args.batch_size, dtype=tf.float32)
      feed[self.inputs] = prev_inp #(1,1,6)
      ...
      if not gmm:
        ...
        acc = outputs[0][0, :] #???? outputs[0].shape = (time, 3)??????????????    
        state = outputs[1:] 
      else:
        outputs = sess.run(
          [self.pi, self.mu, self.sigma, self.corr] + nest.flatten(self.state_out),
